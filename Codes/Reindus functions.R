...
{ 
  # On d?mare par calculer le matrices sources dont on a besoin pour les linkages
  if(OptMRIO==TRUE){
  ...
  L_dt<-L_dt[PR==BR,value:=0] # On met la diagonale ? 0 pour avoir un indicateur de total linkage
  L_dt<-SplitPRBR(L_dt)
  ...
  G_dt<-G_dt[PR==BR,value:=0] # On met la diagonale ? 0 pour avoir un indicateur de total linkage
  G_dt<-SplitPRBR(G_dt)
  
  # Sommation : num?rateur
  Bwd_num<-L_dt[, sum(value),by=c("Lig_Country","Lig_Indus")]
  Fwd_num<-G_dt[, sum(value),by=c("Col_Country","Col_Indus")]
  
  # Sommation : d?nominateur
  ...
  # Calcul des indicateurs normalis?s (par les d?nominateurs)
...
# Fonction de d?composition GAO (offshoring/reshoring/reoffshoring)
...
{ 
  ...
  # Recuperation des caract?ristiques de format du mrio consid?r?
  ...
  # Calcul (par agr?gation) de c0star et c1star et de leur version matricielle par empilement
  ...
  # r1=[C0* x r1A] -c0 : relocalisation sectorielle tir?e par les inputs (variation li?e ? un changement de fournisseur )
  ...
  # Part de la conso finale fournies par chaque ?conomie (cas 2 produits/ 2 pays)
  ...
  # r2=(Fc1-Fc0)*ff1   ( relocalisation sectorielle tir?e par les produits finaux  )
  ...
  # r3=((I-A0)^1-1)*r2*e  (  relocalisation sectorielle indirecte tir?e par les produits finaux  )
  ...
  # Transformation en DT et base de r?sultat
...
# Fonction boucle pays et ann?es sur les contenus en VA des exports
...
# Fonction de calcul du contenu en VA des eXports pour un pays donn? et une nn?e donn?e
ContVAdesExports <- function(DT,annee,pays,OptMRIO=TRUE)  # OptMRIO si on est sur A17 ou ECOLE
{
  ...
  # Cadre de d?part pour y placer les composantes du calcul (m?me format que la DF)
  Cadre<-Base_depart[["DF"]]
  Cadre$value<-0
  Cadre<-ReqSum(Cadre,"Col_Indus",OptMRIO=OptMRIO) # on agr?ge les composantes de demande finale
  ...
  BaseSelect<-ReqSum(BaseSelect,c("Col_Country","Col_Indus"),OptMRIO=OptMRIO) # on agr?ge les composantes de demande finale
  BaseDom<-BaseSelect
  BaseDom$Col_Country<-pays
  BaseDom$Col_Indus<-"TOTDF"
  
  # Calcul des (en n?gatif) des exports des autres pays vers FRA
  BaseSelect<-BaseCIDF[Lig_Country!=pays & Col_Country==pays,]
  BaseSelect<-ReqSum(BaseSelect,c("Col_Indus"),OptMRIO=OptMRIO) # on agr?ge les composantes de demande finale
  ...
  # Rajout des composantes au cadre et agr?gation pour int?grer les composantes au cadre
  Cadre$Partie<-"initial"
  BaseDom$Partie<-"BaseDom"
  BaseXversDom$Partie<-"BaseXversDom"
  BaseTot<-rbind(Cadre,BaseDom,BaseXversDom)
  BaseFin<-ReqSum(BaseTot,c("Partie"),OptMRIO=OptMRIO)
  Cadre<-BaseFin
  Cadre_tab<-dcast(Cadre, Lig_Country + Lig_Indus ~ Col_Country, value.var = "value") 
  ...
  # Ajout des donn?es d'emportations et de VA  pour d?nominateur des ratio
  BaseCIDF<-rbind(Base_depart[["CI"]],Base_depart[["DF"]])
  ...
  Base_Out<-rbind(Base_Out,BaseExport,BaseVA) # Ajout des donn?es d'export et de VA pour les d?nominateurs d'indicateurs ratio
  return(Base_Out)
}  


# Fonction de requetage pour caluler une somme ? partir d'une liste des dimensions ? sommer
...
# Fonction boucle ann?es sur les MADE-IN
...
MadeIn<- function(base_dt,annee,OptMRIO=TRUE)  
{
  ...
  # Calcul du ratio Made-In ventil? par branches
  ...
  ratio_parBR<-ratio_parBR[,..Ordre_Col]  # selection des colonnes qui nous int?ressent
  ...
  ratio_Tot<-ratio_Tot[,..Ordre_Col]  # selection des colonnes qui nous int?ressent
...
# Fonction de calcul des contenus (en VA, ?missions CO2, emploi)
Contenus<- function(List_dt,typeContenu,MethContenu="MatDF")  
  # typeContenu=VA;CO2;Emploi   MethContenu=MatDF;DiagDFtot
{
  ...
  # Identification et r?cup?ration du stressor
  if(typeContenu=="VA"){
    stressor<-VA
  }
  if(typeContenu=="CO2"){
  ...
  if(MethContenu=="DiagDFtot"){
    part3<-as.matrix(diag(DF_TOT$value))
  }
...
#  MatEmpreinte_dt$Lig_Country<-str_extract(MatEmpreinte_dt$joint, "[^_]+")
#  MatEmpreinte_dt$Lig_Indus<-substring(str_extract(MatEmpreinte_dt$joint, "_.+$"),2)
#  MatEmpreinte_dt$Col_Country<-str_extract(MatEmpreinte_dt$variable, "[^_]+")
#  MatEmpreinte_dt$Col_Indus<-substring(str_extract(MatEmpreinte_dt$variable, "_.+$"),2)
#  MatEmpreinte_dt<-MatEmpreinte_dt[,joint:=NULL]
#  MatEmpreinte_dt<-MatEmpreinte_dt[,variable:=NULL]
...
{
  #	On place ? 0 la ligne et la colonne concern?e par l'extraction, mais ? la fa?on Dietzenbacher en compensant
  #    les importations des autres pays en ce produit par un switch sur les autres produits 
  #	Les calculs sont transversaux au TEI et FD : c'est tout le produit qui est concern? uniform?ment.
  
  Base_init<-dt_mrio[year==annee,]
  
  # Extraction des composantes n?cessaires pour le calcul final de l'indicateur HEM
  if(OptMRIO==TRUE){
  ...
  #	1/ On extrait le Lig_Indus de Lig_Country concern? et on le sanctuarise. On 
  #        obtient un montant ? ventiler pour chaque colonne du MRIO.
  ...
  #	3/ On calcule directmeent en requ?tes le total puis la structure (on le fait directement par 
  ...
  if(verboseCheck==TRUE){ # check : On verifie que la structure somme bien ? 1
    print("On verifie que la structure somme bien a 1 :")
  ...
  #     ATTENTION : On ne les convertit pas en n?gatif contrairement ? HRM. On obtient les montants ? additionner aux donn?es du MRIO.
  ...
  Lig_Ventil_delta<-Lig_Ventil_delta[,..Ordre_Col]  # selection des colonnes qui nous int?ressent
  ...
  #	5/ On ajoute (rbind) la base initiale avec la base du point 1 (ce qui est ? ajouter) et la base du 
  #        point 4 (ce qui est ? retrancher)
  #     On cr?e dans un premier temps la base des delta et on contr?le que les ajouts sont biens 
  #     compens?s par les retraits
  ...
  #	6/ On somme suivant l'ensemble des dimensions du MRIO hormis la variable de statut : ?a fait donc 
  #       automatiquement les sommes des montants du point 1 et ?a retranche la ventilation du point 4.
  ...
  # 6bis/ in fine on place la ligne et la colonne extraite ? 0 (ce qui conserve la nullit? de la somme 
  ...
  if( OptAmontAval=="Amont"){ # on ne place que la colonne extraite ? 0
    Base_HEM[Col_Indus==extract_indus & Col_Country==extract_country,"value"]<-0
  }
  if( OptAmontAval=="Aval"){ # on ne place que la ligne extraite ? 0
    Base_HEM[Lig_Indus==extract_indus & Lig_Country==extract_country,"value"]<-0
  }
  
  #	7/ Sortie sous forme de tableau, avec ?ventuel contr?le (on fait la diff?rence entre le MRIO initial et 
  #      le final, et on calcule le colSums pour v?rifier que c'est bien nul.
  ...
  if(verboseCheck==TRUE){ # Check : On calcule le colSums des corrections pour v?rifier que c'est bien nul
    print("On calcule le colSums des corrections pour v?rifier que c'est bien ? 0 :")
...
{
  #	On impl?mente un choc de repat_pct % sur le produit repat_indus pour le pays repat_country : 
  ...
  #	1/ On extrait le Lig_Indus de Lig_Country concern? et on calcul repat_pct % de cette ligne. On 
  ...
  #	2/ On extrait les Lig_Indus de tous les pays hors le pays choqu? en ligne et hors parties 
  #        domestiques (Lig_country==Col_Country)
  ...
  if(verboseCheck==TRUE){ # check : On verifie que la structure somme bien ? 1
  print("On verifie que la structure somme bien ? 1 :")
    if(OptMRIO==TRUE){ print(Lig_Ventil_totaux[,sum(value),by=.(year,Lig_Indus,Col_Indus,Col_Country,MRIO)]) 
    }else{print(Lig_Ventil_totaux[,sum(value),by=.(year,Lig_Indus,Col_Indus,Col_Country)]) }
  }
  
  #	4/ Ensuite on ventile le montant du point 1 sur la structure du point 3. On les convertit en n?gatif 
  #     car il faudra les retrancher. On obtient les montants ? retrancher aux donn?es du MRIO.
  Lig_Ventil_delta<-Delta[Lig_Ventil_totaux,on=.(Lig_Indus,Col_Country,Col_Indus)]
  Lig_Ventil_delta<-Lig_Ventil_delta[,value_delta:=-value*i.value]
  if(OptMRIO==TRUE){  Ordre_Col<-c("year","i.Lig_Country","Col_Country","Lig_Indus","Col_Indus","value_delta","MRIO") 
  }else{ Ordre_Col<-c("year","i.Lig_Country","Col_Country","Lig_Indus","Col_Indus","value_delta")}
  ...
  if(verboseCheck==TRUE){ # Check : On verifie que la structure somme bien ? 0
    print("On verifie que la structure somme bien ? 0 :")
  ...
  if(verboseCheck==TRUE){ # Check : On calcule le colSums des corrections pour v?rifier que c'est bien nul
    print("On calcule max( colSums ) des corrections pour v?rifier que c'est bien ? 0 :")
...
# S?lection d'une composante du mrio complet
   #  Attention : ici on supprime la r?f?rence au MRIO car on n'est plus sur des bases communes ? tous les MRIO mais plut?t individuelles
CompoMRIO<- function(dt_MRIO,typeCompo,date=9999,country="ALL",OptTab=FALSE )  # options: =CI:DF;PROD;VA;A;B;CI_PR;CI_BR;DF_TOT?;L;invB?;OptFullOptions?;OptFullOptionsBonus?: les deux derni?res options sortent des listes avec l'ensemble des sorties individuelles (le version 'Bonus' est exhaustive mais plus longue car inversions matricielles
{
  interm<-dt_MRIO
  ...
  if(typeCompo=="PROD"){ # Recalcul a la volee a partir du MRIO pour eviter les incoh?rences
  ...
  if(typeCompo=="B"){ # Calcul des coefficients de d?bouch?s
    Part_CI<-CompoMRIO(interm,"CI",date=date)
    Part_Prod<-CompoMRIO(interm,"PROD",date=date)
  ...
  if(typeCompo=="CI_PR"){ # Recalcul a la volee a partir du MRIO pour eviter les incoh?rences
    interm<-CompoMRIO(interm,"CI",date=date)
    interm<-interm[,.(sum(value)), by=c("year","Lig_Indus","Lig_Country")]
    setnames(interm, "V1", "value")
    setorder(interm,Lig_Country,Lig_Indus)
  }
  if(typeCompo=="CI_BR"){ # Recalcul a la volee a partir du MRIO pour eviter les incoh?rences
    interm<-CompoMRIO(interm,"CI",date=date)
    interm<-interm[,.(sum(value)), by=c("year","Col_Indus","Col_Country")]
    setnames(interm, "V1", "value")
    setorder(interm,Col_Country,Col_Indus)
  }
  if(typeCompo=="DF_TOT"){ # Recalcul a la volee a partir du MRIO pour eviter les incoh?rences
  ...
  if(typeCompo=="L"){ # Calcul de l'inverse de L?ontief
...
# S?lection d'une composante du mrio ECOLE ou A17 (attention priorit? par d?faut ? Figaro sur 2010-2014)
CompoECOLEouA17<- function(dt_MRIO,typeCompo,date=9999,country="ALL",OptTab=FALSE,OptMRIO1014="FIGARO",OptMRIO2000="WIOD" )  #options: =CI:DF;PROD;VA;A;B;CI_PR;CI_BR;DF_TOT?;L;invB?;OptFullOptions?;OptFullOptionsBonus?: les deux derni?res options sortent des listes avec l'ensemble des sorties individuelles (le version 'Bonus' est exhaustive mais plus longue car inversions matricielles
{
  ...
  if(date>=2010 & date<=2014){ # Cas ou on a 2 bases mrio possibles sur la p?riode : on privil?gie FIGARO par d?faut mais on conserve la possibilit? de r?cup?rer WIOD
    if(OptMRIO1014!="FIGARO"){interm<-interm[MRIO=="WIOD",]}else{interm<-interm[MRIO=="FIGARO",]}
  }  
  if(date==2000){ # Cas ou on a 2 bases mrio possibles en 2000 : on privil?gie WIOD par d?faut mais on conserve la possibilit? de r?cup?rer LR-WIOD
...
# Ajout de pr et BR par concat?nation
...
SplitPRBR<- function(dt)  
{
 ...
 # Gestion du cas particulier UE_OTHERS (le seul a avoir un underscore qui est malheureusement utilis? comme delimiteur)
...
# Somme par produit du TEI ou autre df, vectoris?  // utilise la fonction vectDF d?finie par ailleurs
TotPR<- function(df)  
{
...
# Count negat dans le TES  // utilise la fonction CountNegat d?finie par ailleurs
...
# Fonction de check de coh?rence des ERE
...
Ventil_VectsurTEI<- function(vect,DFtei)  # Options : "Classic" ou "TESS" : Attzention les noms de colonne diff?rent.
{
...
Recalcuc_Totaux_TEF<- function(df,ci=NULL,Typ_TEF="Classic")  # Options : "Classic" ou "TESS" ou "TESS_A48" : Attention les noms de colonne diff?rent.
{
  if(Typ_TEF=="Classic"){
    # Recalcul des agr?gats de la demande finale (type FBC)
    df<-GereInfNA(df)
...
GereInfNA <- function(df,impute=0){  # Fonction de contr?le de la coh?rence comptable.
...
CountNegat <- function(df)  # Fonction de contr?le de la coh?rence comptable.
{
  df<-as.data.frame(df)
  df_w<-df
  ...
  FirstTab<-AddRownamesToFirstCol(df_w) # rajoute le nom des lignes dans la première colonne
...
if((dim(tab1)[1]!=dim(tab2)[1]) | (dim(tab1)[2]!=dim(tab2)[2])){return(print("Error dimensions inconsistent"))}
  ...
  FirstTab<-AddRownamesToFirstCol(FirstTab) # rajoute le nom des lignes dans la première colonne
  colnames(FirstTab)[1]<-"Produit"
  Cadre<- FirstTab %>% tidyr::pivot_longer(cols=!Produit, 
                                           names_to = "ColumnTab", 
                                           values_to = "tab1")
  ...
  # Traitement du second tableau
    ...
    Tab<-AddRownamesToFirstCol(Tab) # rajoute le nom des lignes dans la première colonne
...
CountNAInf <- function(df)  # Fonction de contr?le de la coh?rence comptable.
{
  df<-as.data.frame(df)
  is.na(df)<-sapply(df, is.infinite)
  interm<-sum(is.na(df))
...
CheckCAFAB <- function(produit)  # Fonction de contr?le de la coh?rence comptable.
{
...
AgregNivG_toA64A48_1D <- function(df, transp=FALSE,BoolPCHRCAFAB=FALSE)
{
  ...
  AgregA48<-TabJoined[,-2] %>% 
    group_by(A48_CT) %>% 
    summarise_at(vars(2:nbcol),sum, na.rm = TRUE)
  AgregA64<-AgregA64[-67,] # Retirer la dernière ligne qui est nulle
  AgregA48<-AgregA48[-51,] # Retirer la dernière ligne qui est nulle
  AgregA64<-GetRownamesFromFirstCol(AgregA64)
  AgregA48<-GetRownamesFromFirstCol(AgregA48)
  
    SelecPCHTRCAFAB64<-AgregA64[c('PCHTR','PCAFAB'),] #On extrait les 2 lignes qui ne sont pas au bon endroit afin de les réintroduires à la fin ensuite.
    AgregA64<-AgregA64[-c(55,56),]
    SelecPCHTRCAFAB48<-AgregA48[c('PCHTR','PCAFAB'),] #On extrait les 2 lignes qui ne sont pas au bon endroit afin de les réintroduires à la fin ensuite.
    AgregA48<-AgregA48[-c(43,44),]
    
    if(isTRUE(BoolPCHRCAFAB)){  # Si correct territoriale et CAFAb présents : necessiter de reordonner les DF agreges pour placer les 2 lignes a la fin dans le bon ordre
...
AgregNivA48_toH45_1D <- function(df, transp=FALSE,BoolPCHRCAFAB=FALSE)
{
  ...
  TabJoined<-TabJoined[-51,]  # Retirer la dernière ligne qui est nulle
  AgregH45<-TabJoined %>% 
    group_by(H45_CT) %>% 
    summarise_at(vars(2:nbcol),sum, na.rm = TRUE)
 #AgregH45<-AgregH45[-48,] # Retirer la dernière ligne qui est nulle
  AgregH45<-GetRownamesFromFirstCol(AgregH45)
  
  SelecPCHTRCAFAB45<-AgregH45[c('PCHTR','PCAFAB'),] #On extrait les 2 lignes qui ne sont pas au bon endroit afin de les réintroduires à la fin ensuite.
  AgregH45<-AgregH45[-c(40,41),]
  AgregH45<-ReordCodeEtLib(AgregH45)
  AgregH45<-AgregH45[-c(46,47),] # On retire PCHTR et CAFAB qui sont ajout?s par le r?ordonnancement
...
{ #Attention : on suppose que les tables de reordonnancement et de libelles sont déjà chargées, sinon les charger ici.
  df<-as.data.frame(df)
  nblig<-nrow(df)
  
  if(nblig<=47){ #Détection niveau H45 
    df2<-AddRownamesToFirstCol(df)
    nbcol<-ncol(df2)
  ...
  if(nblig>47 & nblig<55){ #Détection niveau A48 
    df2<-AddRownamesToFirstCol(df)
    nbcol<-ncol(df2)
    df3<-AddChrToFirstColRownames(df2,"F")
    colnames(df3)[1]<-"A48"
  ...
  if(nblig>55 & nblig<75){ #Détection niveau A64 
    df2<-AddRownamesToFirstCol(df)
    nbcol<-ncol(df2)
    colnames(df2)[1]<-"A64"
  ...
  if(nblig>75){ #Détection niveau A138 
    df2<-AddRownamesToFirstCol(df)
    nbcol<-ncol(df2)
    df3<-AddChrToFirstColRownames(df2,"G")
    colnames(df3)[1]<-"A138"
...
#  THEIL Accuracy and Quality  : Theil’s U (Theil, 1971). When U = 0 the forecasts are perfect. That means if U>0 then forecasts are not perfect. As we keep increasing U, the forecasts will keep becoming more and more imperfect. When it reaches 1, it means Forecast is equivalent to Naive Method. Hence, a general pattern can be seen that as U increases forecast is wrong and at 1 forecast is same as naive and hence by extrapolating this we can say that if U>1 then the forecast is worse than naive method forecast.
#________________________________________________
...
#________________________________________________
#  MAE : The Mean Absolute Error (MAE) is good to measure forecast accuracy. As the name implies, it is the mean of the absolute error. -> Inconvénient de la moyenne mais avantage de ne pas être affecté par les faibles montants contrairement à MAPE. A privilégier rpar rapport à MAPE.
#________________________________________________
#  STPE  : Standardized Total Percentage Error (STPE; Miller and Blair, 1985). Version améliorée de MAPE dans notre cas car ce ne sont pas les écarts relatifs mais les abs(écart) qui sont comparés au total du niveau (homogénéisation). Ne fournit pas d'informations sur la distribution des écarts (a compléter par RMSE).
#________________________________________________
#  RMSE%  : The Root Mean Squared Error (RMSE) is defined as the square root of the average squared error. -> Avantage pour détecter les gros écarts qui surpondèrent l'indicateur car mis au carré. => Permet d'identidier si on a bcp de petits écarts ou plutôt quelques grands écarts.
#________________________________________________
#  guide de lecture  : Privilégier les 2 THEIL's U, s'appuyer sur STPE pour avoir l'ampleur des écarts et sur RMSE% pour avoir une idée de la distribution des écarts (si important => certains gros écarts probablement)
...
# Attention : tous les tableaux doivent avoir strictement le même format
...
#################################################################################
##   AVIONIC :  Variation du prix de production en fonction d'un choc exogène de prix    ##
#################################################################################
